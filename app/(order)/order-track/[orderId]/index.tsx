...
type OrderStatus = 'pending' | 'shipped' | 'completed' | 'canceled'
  ...
  useEffect(() => {
    const fetchOrderStatus = async () => {
      try {
        ...
        setStatus(
          allowedStatuses.includes(apiStatus)
            ? apiStatus
            : 'pending'
        )
      } catch (error) {
  ...
  return (
    <SafeView className="flex-1 bg-[#F8FAFC]">
      <ScrollView className="flex-1 px-6 pt-8">
        ...
        {/* CANCELED STATE */}
        {isCanceled && (
          <View className="bg-red-50 border border-red-200 rounded-[28px] px-6 py-8 mb-6">
            <Text
            ...
            <Text
              ...
              style={{ writingDirection: 'rtl' }}
            >
              لا يمكن متابعة هذا الطلب لأنه تم إلغاؤه
            </Text>
          </View>
        )}
